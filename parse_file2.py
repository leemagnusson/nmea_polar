...
prog = re.compile(r'\$(?P<Talker>\w{2})(?P<Type>\w{3})')
progs = {'MWV': re.compile(r'\$\w{2}MWV,' + \
                r'(((?P<WindAngleRelative>[\d\.]*),R,(?P<WindSpeedRelative>[\d\.]*))' + \
                r'|((?P<WindAngleTrue>[\d\.]*),T,(?P<WindSpeedTrue>[\d\.]*)))'), \
         'VHW': re.compile(r'\$\w{2}VHW,' + \
                r'(?P<WaterHeadingDegreesTrue>[\d\.]*),T,(?P<WaterHeadingDegreesMagnetic>[\d\.]*),M,' + \
                r'(?P<SOWKnots>[\d\.]*),N'), \
         'GGA': re.compile(r'\$\w{2}GGA,'  +\
                r'(?P<UTC>\d{6}),' + \
                r'(?P<Latitude>[\d\.]*),(N|S),' + \
                r'(?P<Longitude>[\d\.]*)' )}
...
def parse(s):
    ...
    if result:
        d = result.groupdict()
        if result.group('Type') in progs.keys():
         #   print s
            result2 = progs[result.group('Type')].match(s)
          #  print result2
          #  print result2.groupdict()
            if result2:
                d.update(result2.groupdict())
          #  print d
        return d
    else:
        return {}
...
fields = [('UTC',str),('Latitude',float),('Longitude',float),
          ('Speed Knots',float),('Magnetic Heading',float),
          ('WindAngleRelative',float), ('WindSpeedRelative',float),
          ('SOWKnots',float)]
